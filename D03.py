...
# Function to display overview analytics
def show_overview(df):
    ...
    fig_race = px.pie(df, names='What is your race?', title='Race Distribution')
    st.plotly_chart(fig_race, use_container_width=True)
    
    # Display age distribution dot plot
    st.subheader("Age Distribution")
    fig_age = px.scatter(df, x='How old are you?', title='Age Distribution')
...
page_selection = st.sidebar.selectbox(
    "Select Page",
    ["Overview Analytics", "Event Details"]
)

if page_selection == "Overview Analytics":
    show_overview(df)
else:
    ...
    if chart_type == "Race Distribution":
        ...
        st.plotly_chart(fig, use_container_width=True, height=800)
        fig_dotplot = px.scatter(df, x='How old are you?', title='Age Distribution')
